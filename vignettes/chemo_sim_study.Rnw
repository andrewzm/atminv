...
We will now set up our simulation. Here we will only be concerned with the model `full' (please ignore the other settings), which can be misspecified (\texttt{misspecification = 1}) or not (\texttt{misspecification = 0}).
<<>>=
###------------------
### Parameters
###------------------
