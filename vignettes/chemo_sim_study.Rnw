...
print(LinePlotTheme() + geom_tile(data=st_grid,aes(s,t,fill=p)))
@
\noindent the result of which is depicted in Figure \ref{fig:upsilon}

\begin{figure}
\begin{center}
<<upsilon-plot2,fig.height=3,fig.width=3>>=
print(ggplot(st_grid) + geom_tile(aes(s,t,fill=p)))
@
\end{center}
...
In this section we show the code for carrying inference on the bivariate field $(\Yvec_f,\Yvec_m)'$. We first compute the mean flux density 
