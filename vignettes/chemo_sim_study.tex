...
%\VignetteIndexEntry{Reproducible code for Section 4.1}
...
\hlstd{save_images} \hlkwb{<-}   \hlnum{1}  \hlcom{# Development only}
...
\end{alltt}
\end{kframe}
\includegraphics[width=\maxwidth]{figure/upsilon-plot2} 
...
\hlstd{S_f_log} \hlkwb{<-} \hlkwd{variogramLine}\hlstd{(variogram_model,} \hlcom{# Find covariance matrix Sigma_f}
                         \hlkwc{dist_vector} \hlstd{= sp::}\hlkwd{spDists}\hlstd{(}\hlkwd{matrix}\hlstd{(s_axis),}
...
\hlstd{\}} \hlkwa{else if}\hlstd{(model} \hlopt{==} \hlstr{"full_big"}\hlstd{) \{}
    ...
    \hlstd{corr_s_mat} \hlkwb{<-} \hlkwa{function}\hlstd{(}\hlkwc{theta_s}\hlstd{) atminv:::}\hlkwd{corr_s}\hlstd{(}\hlkwc{s} \hlstd{= s_axis,}\hlkwc{theta_s} \hlstd{= theta_s)}
    \hlstd{corr_t_mat} \hlkwb{<-} \hlkwa{function}\hlstd{(}\hlkwc{theta_t}\hlstd{) atminv:::}\hlkwd{corr_t}\hlstd{(}\hlkwc{t} \hlstd{= t_axis,}\hlkwc{theta_t} \hlstd{= theta_t)}
    \hlstd{d_corr_s_mat} \hlkwb{<-} \hlkwa{function}\hlstd{(}\hlkwc{theta_s}\hlstd{) atminv:::}\hlkwd{d_corr_s}\hlstd{(}\hlkwc{s} \hlstd{= s_axis,}\hlkwc{theta_s} \hlstd{= theta_s)}
    \hlstd{d_corr_t_mat} \hlkwb{<-} \hlkwa{function}\hlstd{(}\hlkwc{theta_t}\hlstd{) atminv:::}\hlkwd{d_corr_t}\hlstd{(}\hlkwc{t} \hlstd{= t_axis,}\hlkwc{theta_t} \hlstd{= theta_t)}
...
\hlkwa{if}\hlstd{(model} \hlopt{==} \hlstr{"full"}\hlstd{) \{}
    ...
    \hlkwa{if}\hlstd{(save_images)}
      \hlkwd{ggsave}\hlstd{(g,}\hlkwc{filename} \hlstd{=} \hlstr{"../img/Sim_plot.png"}\hlstd{,}\hlkwc{width}\hlstd{=}\hlnum{10}\hlstd{,}\hlkwc{height}\hlstd{=}\hlnum{4}\hlstd{)}
\hlstd{\}}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\end{alltt}
\end{kframe}
\includegraphics[width=\maxwidth]{figure/YfYm-plot} 
...
\hlcom{# TRUE B}
\hlstd{B_true} \hlkwb{<-} \hlstd{plyr::}\hlkwd{ddply}\hlstd{(df_for_B,}\hlkwd{c}\hlstd{(}\hlstr{"t"}\hlstd{,}\hlstr{"s"}\hlstd{),}\hlkwa{function}\hlstd{(}\hlkwc{df}\hlstd{) \{}
...
\hlstd{B_true_df} \hlkwb{<-} \hlstd{plyr::}\hlkwd{ddply}\hlstd{(df_for_B,}\hlkwd{c}\hlstd{(}\hlstr{"t"}\hlstd{,}\hlstr{"s"}\hlstd{),}\hlkwa{function}\hlstd{(}\hlkwc{df}\hlstd{) \{}
  \hlstd{b} \hlkwb{=} \hlkwd{b}\hlstd{(}\hlkwc{s}\hlstd{=df}\hlopt{$}\hlstd{s[}\hlnum{1}\hlstd{],}\hlkwc{u}\hlstd{=s_axis,}\hlkwc{p}\hlstd{=df}\hlopt{$}\hlstd{p[}\hlnum{1}\hlstd{])\})} \hlopt{%>%}
...
\end{alltt}
\end{kframe}
\includegraphics[width=\maxwidth]{figure/B-plot} 
...
\hlkwa{if}\hlstd{(cache_results} \hlopt{&} \hlstd{model} \hlopt{==} \hlstr{"full_big"}\hlstd{) \{}
    \hlstd{X}\hlopt{$}\hlstd{lap_approx} \hlkwb{<-} \hlkwa{NULL} \hlcom{# Save space}
    \hlkwd{save}\hlstd{(X,}\hlkwc{file}\hlstd{=}\hlkwd{paste0}\hlstd{(}\hlstr{"../inst/extdata/"}\hlstd{,filename))}
\hlstd{\}}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\end{alltt}
\end{kframe}
\includegraphics[width=\maxwidth]{figure/EM-plot} 
...
\begin{alltt}
\hlkwa{if}\hlstd{(model} \hlopt{==} \hlstr{"full"}\hlstd{) \{}
  \hlstd{corr_zeta} \hlkwb{<-} \hlkwd{corr_zeta_fn}\hlstd{(}\hlkwd{c}\hlstd{(s_obs}\hlopt{$}\hlstd{s[}\hlnum{1}\hlopt{:}\hlstd{(m_obs}\hlopt{-}\hlnum{1}\hlstd{)],new_obs),}
                            \hlstd{t_axis,X}\hlopt{$}\hlstd{theta[}\hlnum{2}\hlstd{,n_EM],X}\hlopt{$}\hlstd{theta[}\hlnum{3}\hlstd{,n_EM])}
...
\begin{alltt}
 \hlstd{Q} \hlkwb{<-} \hlkwd{as.data.frame}\hlstd{(}\hlkwd{t}\hlstd{(q[}\hlnum{1}\hlopt{:}\hlkwd{length}\hlstd{(X}\hlopt{$}\hlstd{ind),}\hlopt{-}\hlkwd{c}\hlstd{(}\hlnum{1}\hlopt{:}\hlnum{1000}\hlstd{,i)]))} \hlopt{%>%}
  \hlstd{tidyr::}\hlkwd{gather}\hlstd{(t,z)} \hlopt{%>%}
...
\end{alltt}
\end{kframe}
\includegraphics[width=\maxwidth]{figure/HMC-plot} 
...
\hlkwa{if}\hlstd{(}\hlopt{!}\hlstd{misspecification} \hlopt{&} \hlstd{model}\hlopt{==}\hlstr{"full"} \hlopt{&} \hlstd{save_images) \{}
  \hlkwd{png}\hlstd{(}\hlstr{"../img/density_sim1.png"}\hlstd{,} \hlkwc{width}\hlstd{=}\hlnum{4}\hlstd{,} \hlkwc{height}\hlstd{=}\hlnum{4}\hlstd{,} \hlkwc{units}\hlstd{=}\hlstr{"in"}\hlstd{,} \hlkwc{res}\hlstd{=}\hlnum{300}\hlstd{)}
  ...
  \hlkwd{png}\hlstd{(}\hlstr{"../img/density_sim10.png"}\hlstd{,} \hlkwc{width}\hlstd{=}\hlnum{4}\hlstd{,} \hlkwc{height}\hlstd{=}\hlnum{4}\hlstd{,} \hlkwc{units}\hlstd{=}\hlstr{"in"}\hlstd{,} \hlkwc{res}\hlstd{=}\hlnum{300}\hlstd{)}
