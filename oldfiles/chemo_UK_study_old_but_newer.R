...
read_process_models <- function(des,prune=T) {
    ...
    for(i in 1:length(des$model_filenames)) {
        Model_B_list[[i]] <- read.table(des$model_filenames[i],skip = 15) %>%
            cbind(select(yx,-t,-UK_territory,-Ir_territory,-z)) %>%
...
Emissions.fit2 = fit.variogram(Emissions.vgm, model = vgm(1,model= "Exp",1,1))
Emissions.fit3 = fit.variogram(Emissions.vgm, model = vgm(1,model= "Gau",1,1))
## The attr(Emissions.fit,"SSErr") of each one shows that the Spherical model has the lowest sum of squared errors
...
load("~/Desktop/Chemometrics_results/test_idx.rda")
C_test <- C[test_idx,]
...
save(X,file="~/Desktop/Chemometrics_results/UK_results5.dat")
i <- 55
...
save(list=ls(),file="~/Desktop/Chemometrics_results/UK_results_all5.dat")
...
## KS test (not complete)
stat1 <- seq(i,4320,by=4)
mu <- apply(mf_samp[stat1,-c(1:1000,i)],1,median)
KS_df <- data.frame(mu = mu, t = 1:length(mu))
O <- subset(s_obs,obs_name==s_obs[1,]$obs_name) %>%
    left_join(KS_df)
...
ver <- crps(pred_log["log.z"][,1],cbind(pred_log["log.z.pred"],sqrt(pred_log["log.z.var"])))
hist(ver$crps)

library(fitdistrplus)
res_norm <- (pred_log["log.z"] - pred_log["log.z.pred"])/sqrt(pred_log["log.z.var"])
png("../norm_resid_log_flux.png", width=10, height=8, units="in", res=300)
#qqnorm(res_norm[,1],xlim=c(-3,3)); qqline(res_norm[,1]); dev.off()
plotdist(res_norm[,1],"norm",para=list(mean=0,sd=1)); dev.off()


# res_norm2 <- (pred_log["z"] - pred_log["z.pred"])/sqrt(pred_log["z.var"]) 
