...
### Parameters
###------------------
model = "full" ## Either sparse or full or diag
misspecification = 0
...
if(model == "full") {
    ...
    g <- LinePlotTheme() + geom_line(data=X,aes(x=s,y=value,linetype=as.factor(process)))+ 
        geom_segment(data=s_obs[1:5,],aes(x=s, xend=s, y = 0, yend = 50),
                     arrow=arrow(length=unit(0.1,"cm"))) +
        ylab("") +
        scale_linetype_discrete(guide=guide_legend(title="process"))
    ggsave(g,filename = "../../MVCF/Chemometrics/Sim_plot.png",width=10,height=4)
}
...
#     coord_fixed(xlim=c(-10,10),ratio = 0.2)
  ...
  ggsave(filename = "../../MVCF/Chemometrics/B_plot.png",width=12)
} 
...
# Ym <- rnorm(ns*nt)
# Yf <- mu_f
# Qf_log <- solve(S_f_log)
# Q_zeta_s <- Q_s_mat()
# Q_zeta_t <- Q_t_mat(theta_t_true)
# 
# Q_zeta_s_dense <- as.matrix(Q_zeta_s)
# Q_zeta_t_dense <- as.matrix(Q_zeta_t)
# 
# Q_zeta_x <- function(X) {
#   (1/sigma_zeta_true^2) * as.vector(Q_zeta_s %*% matrix(as.vector(X),ns,nt) %*% Q_zeta_t)
# }
# 
# Q_zeta_x2 <- function(X) {
#     (1/sigma_zeta_true^2) * as.vector(Q_zeta_s_dense %*% matrix(as.vector(X),ns,nt) %*% Q_zeta_t_dense)
# }
# 
# 
# -0.5 * crossprod(chol(Qobs) %*% (s_obs$z - C_m %*% Ym)) -
#     0.5 * t(Ym - B %*% Yf) %*% Q_zeta_x(Ym - B %*% Yf) -
#     0.5 * crossprod(chol(Qf_log) %*% (log(Yf) - mu_f_log)) - 
#     sum(log(Yf))
...
if(model == "full") {
  ...
  S_zeta <- X$theta[1,n_EM] * Imat(nrow(s_obs))
...
#  geom_point(data=subset(s_obs,t==1),aes(x=as.factor(s),y = 0),colour='blue',size=3) +
  coord_fixed(ratio = 0.03, ylim=c(0,1050)) + ylab("Yf")
g
...
comp_density <- function(j,xu,xl=0,yu=0.01) {
  x <- seq(xl,xu,by=1)
  hist(q[j,1:(N-1)],xlab=c("flux (g/s)"),ylab="[Yf | Zm]",main="",freq=F,xlim=c(xl,xu),ylim=c(0,yu))
  lines(x,dnorm(x,mean=X$lap_approx$Yf[j],sd=sqrt(X$lap_approx$S_ff[j,j])),lty=2)
}

png("../../MVCF/Chemometrics/density_sim1.png", width=4, height=4, units="in", res=300)
comp_density(70,800,-200,yu=0.005); dev.off()
png("../../MVCF/Chemometrics/density_sim10.png", width=4, height=4, units="in", res=300)
comp_density(24,400,50,yu=0.014); dev.off()


# LinePlotTheme() + geom_ribbon(data=data.frame(t = 1:100, 
#                                               y = X$lap_approx$Ym[rm_idx], 
#                                               sd = sqrt(diag(X$lap_approx$S_mm[rm_idx,rm_idx]))),
#                               aes(x = t, y = y, ymax = y + 2*sd, ymin = y - 2*sd),alpha=0.3) +
#                 geom_point(data=filter(st_grid,s == new_obs),aes(x=t,y=Ym)) +
...
print(paste0("RMSPE = ", sqrt(mean(residual^2))))
print(paste0("C1 = ", mean(residual) / (1/length(post_unc) * sqrt(sum(post_unc)))))
print(paste0("C2 = ", sqrt(mean(residual^2) /mean(post_unc))))
...
#                               ind=X$ind)
#     
#     M <- diag(c(1/(X$lap_approx$Yf^2),1/(X$lap_approx$Ym^2)))
#     eps_gen <- function() runif(n=1,min = 0.066, max = 0.068)
#     L <- 10L
#     
#     sampler <- hmc_sampler(U = lap$logf,dUdq = lap$gr_logf, 
#                            M = M,
#                            eps_gen = eps_gen,
#                            L = L,
#                            lower = rep(0,nrow(M)))
...
#     N <- 10000
#     q <- matrix(0,nrow(M),N)
#     qsamp <- c(X$lap_approx$Yf,X$lap_approx$Ym)
#     dither <- 1
#     i <- count <- 1
#     
#     
