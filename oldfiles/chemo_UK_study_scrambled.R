...
ggsave(filename = "../../MVCF/Chemometrics/NAEI.png",plot = g,width=6.4,height=6.7)
...
g <- LinePlotTheme() + geom_tile(data=subset(Emissions,x > -13 & x < 3 & y > 48 & y < 61),
                                 aes(x,y,fill=B))  +
    scale_fill_distiller(palette="BuPu",trans="sqrt",guide = guide_legend(title="sens. (s/g)")) + 
    geom_path(data=Europe,aes(x=long,y=lat,group=group)) +
    coord_map(xlim=c(-12,2),ylim=c(49,60)) + xlab("lon") + ylab("lat") +
...
ggsave(filename = "../../MVCF/Chemometrics/TTA_01_01.png",plot = g,width=6.4,height=6.7)
...
ggsave(filename = "../../MVCF/Chemometrics/TTA_01.png",plot = g,width=10,height=3.7)
...
png("../../MVCF/Chemometrics/histUK.png", width=4, height=4, units="in", res=300)
hist(Emissions$z,main="",xlab="flux (g/s)",ylab="frequency",8); dev.off()
png("../../MVCF/Chemometrics/variogram_est.png", width=4, height=4, units="in", res=300)
plot(Emissions.vgm,Emissions.fit,col="black",xlab=("h (degrees lat/lon)")); dev.off()
...
save(X,file="~/Desktop/UK_results3.dat")
i <- 55
...
save(list=ls(),file="~/Desktop/UK_results_all3.dat")
...
### PLOTTING
Emissions$x_mean <- apply(q[,1:(i-1)],1,median)
Em50 <- LinePlotTheme() + geom_tile(data=Emissions,aes(x,y,fill=pmax(pmin(x_mean,2000),-2000)))  +
  scale_fill_distiller(palette="Spectral",trans="reverse",guide=guide_legend(title = "flux (g/s)")) +
  geom_path(data=Europe,aes(x=long,y=lat,group=group)) +
  coord_map(xlim=c(-12,2),ylim=c(49,60)) + xlab("lon") + ylab("lat")
ggsave(filename = "../../MVCF/Chemometrics/Em50.png",plot = Em50,width=6.4,height=6.7)

Emissions$x_mean <- apply(q[,1:(i-1)],1,quantile,0.95)
Em95 <- LinePlotTheme() + geom_tile(data=Emissions,aes(x,y,fill=pmax(pmin(x_mean,2000),-2000)))  +
  scale_fill_distiller(palette="Spectral",trans="reverse",guide=guide_legend(title = "flux (g/s)",)) +
  geom_path(data=Europe,aes(x=long,y=lat,group=group)) +
  coord_map(xlim=c(-12,2),ylim=c(49,60)) + xlab("lon") + ylab("lat")
ggsave(filename = "../../MVCF/Chemometrics/Em95.png",plot = Em95,width=6.4,height=6.7)
...
png("../../MVCF/Chemometrics/density40.png", width=4, height=4, units="in", res=300)
comp_density(40,78,2000); dev.off()
png("../../MVCF/Chemometrics/density76.png", width=4, height=4, units="in", res=300)
...
print(paste0("Mean UK: ", mean(apply(q[-Ir,1:(i-1)],2,sum))*3600*24*365 + Sea_emissions_UK))
print(paste0("Sd UK: ", sd(apply(q[-Ir,1:(i-1)],2,sum))*3600*24*365))
print(paste0("Inventory UK: ", sum(subset(yx,UK_territory == 1)$z) *3600*24*365))

print(paste0("Mean Ireland: ", mean(apply(q[Ir,1:(i-1)],2,sum))*3600*24*365 + Sea_emissions_Ir))
print(paste0("Sd Ireland: ", sd(apply(q[Ir,1:(i-1)],2,sum))*3600*24*365))
print(paste0("Inventory Ireland: ", sum(subset(yx,Ir_territory == 1)$z) *3600*24*365))

### MOLE FRACTION IN CITIES
...
StCQoz <- S_post %*% (t(C) %*% Qobs %*% s_obs$z)
for (i in 1:N){
...
plot_mf_Jan <- function(i) {
    ...
    g <- LinePlotTheme() + geom_ribbon(data=df,aes(x=t,ymax=uq,ymin=lq),alpha=0.6) +
        ...
        geom_point(data=subset(s_obs,obs_name==s_obs[i,]$obs_name & t < 361),aes(x=t,y = z),colour='black',size=3,shape=4)+ ylab("Ym")
...
g_TTA <- plot_mf_Jan(4) + ggtitle("TTA")
